  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        image: colors.backgroundImagePath != null
            ? DecorationImage(
...
class ThemedButton extends StatefulWidget {
  ...
  final ThemedButtonIconPosition iconPosition;
  final int labelMaxLines;
  final bool forceSingleLineLabel;
  ...
  const ThemedButton({
    ...
    this.iconPosition = ThemedButtonIconPosition.start,
    this.labelMaxLines = 2,
    this.forceSingleLineLabel = false,
    this.contentSpacing = 12,
  }) : assert(
            label != null || child != null, 'Label or Child must be provided');
...
class _ThemedButtonState extends State<ThemedButton>
  ...
  static int _instanceCounter =
      0; // Incrementer to prevent identical seeds on rapid creation
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildLabel() {
      final text = Text(
        ...
        maxLines: widget.forceSingleLineLabel ? 1 : widget.labelMaxLines,
        softWrap: !widget.forceSingleLineLabel,
        overflow: widget.forceSingleLineLabel
            ? TextOverflow.visible
            : TextOverflow.ellipsis,
      );

      if (widget.forceSingleLineLabel) {
        return FittedBox(
    ...
    Widget buildDefaultContent() {
      if (widget.iconPosition == ThemedButtonIconPosition.top) {
        return Column(
    ...
    return GestureDetector(
      ...
      onLongPressStart:
          widget.onLongPress != null ? (_) => _controller.forward() : null,
      onLongPressEnd:
          widget.onLongPress != null ? (_) => _controller.reverse() : null,
        ...
        child: Opacity(
          ...
          child: Container(
            ...
            constraints: const BoxConstraints(
                maxWidth: 400,
                minHeight: 60), // Reduced minHeight, removed maxHeight
            child: CustomPaint(
              ...
              child: Container(
                ...
                alignment:
                    Alignment.center, // Strictly center the child content
                ...
                child: widget.child ?? buildDefaultContent(),
              ),
            ),
          ),
        ),
...
class CyberpunkFramePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        Path()
          ..moveTo(size.width, size.height - cut - 10)
          ..lineTo(size.width, size.height - cut)
          ..lineTo(size.width - cut, size.height)
          ..lineTo(size.width - cut - 10, size.height),
        cornerPaint);
    ...
    // Small rects
    canvas.drawRect(
        Rect.fromLTWH(size.width / 2 - 20, size.height - 4, 40, 2), decorPaint);
...
class GhibliFramePainter extends CustomPainter {
  ...
  void _drawCluster(Canvas canvas, Size size, int position, math.Random random,
      Paint borderPaint) {
    canvas.save();
  ...
  void _drawLeaf(Canvas canvas, math.Random random, double cx, double cy,
      double length, double angleDeg, Paint border) {
    canvas.save();
    ...
    if (roll < 0.7) {
      // Variant of Green
      leafColor = Color.fromARGB(255, 100 + random.nextInt(60),
          180 + random.nextInt(50), 100 + random.nextInt(60));
    } else if (roll < 0.9) {
      // Yellowish
      leafColor = Color.fromARGB(
          255, 220 + random.nextInt(35), 200 + random.nextInt(55), 80);
    } else {
      // Reddish/Berry
      leafColor = Color.fromARGB(255, 200 + random.nextInt(55),
          100 + random.nextInt(50), 100 + random.nextInt(50));
    }
